...
function HourlyForecast({ hours, currentTime, timeZone, units }) {
  ...
  return (
    <section aria-labelledby="hourly-forecast-title" className="flex min-h-0 min-w-0 flex-col xl:absolute xl:inset-0 rounded-3xl border border-white/5 bg-[#111e32] p-5 sm:p-6">
      ...
      {visibleHours.length === 0 ? (
        <p role="status" className="mt-6 text-slate-400">No hourly forecast available for this day.</p>
      ) : (
        <ul ref={listRef} className="mt-5 min-h-0 max-h-[34rem] space-y-2 overflow-y-auto pr-1 xl:max-h-none xl:flex-1 [scrollbar-color:#334155_transparent]">
          {visibleHours.map((hour) => {
            ...
            return (
              <li key={hour.time} className="grid grid-cols-[3rem_2.5rem_minmax(0,1fr)_auto] items-center gap-x-2 rounded-xl bg-[#192b43]/70 px-3 py-3">
                ...
                <p className="col-start-3 row-start-1 text-xs text-sky-300">
                  <span className="sr-only">Chance of precipitation: </span><svg aria-hidden="true" viewBox="0 0 24 24" className="mr-1 inline h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M5 13a7 7 0 0 1 14 0v1H5v-1ZM8 14v2a2 2 0 0 1-4 0M16 14v2a2 2 0 0 0 4 0" /></svg>{Number.isFinite(hour.precipitationProbability) ? `${hour.precipitationProbability}%` : '—'}
                </p>
              </li>
            )
          })}
